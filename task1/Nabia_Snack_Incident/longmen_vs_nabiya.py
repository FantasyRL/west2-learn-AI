    ...
    while nagato_hp>0 and nabiya_hp>0:
        ...
        if action == 'attack':
            ...
            if check_critical_hit(attack_damage):
                print("💥「BIG SEVEN」触发！伤害翻倍！")
                attack_damage *= 2
            effective_damage = max(0, attack_damage - nabiya_defense_bonus)
            nabiya_hp -= effective_damage
            nabiya_defense_bonus=max(0,nabiya_defense_bonus-attack_damage)
            print(f"长门对娜比娅造成了 {effective_damage} 点伤害！")
        elif action == 'defend':
            print("长门选择了防御！")
            nagato_defense_bonus = calculate_defense_value(NAGATO_DEFEND_DICE)
            print(f"长门的防御值提升了 {nagato_defense_bonus} 点！")
        else:  # special
            print("长门使用了特殊攻击！")
            nabiya_hp -= SPECIAL_ATTACK_DAMAGE
            print(f"长门对娜比娅造成了 {SPECIAL_ATTACK_DAMAGE} 点伤害！")
        ...
        if action == 'attack':
            ...
            if check_critical_hit(attack_damage):
                print("💥「BIG SEVEN」触发！伤害翻倍！")
                attack_damage *= 2
            effective_damage = max(0, attack_damage - nagato_defense_bonus)
            nagato_hp -= effective_damage
            nagato_defense_bonus=max(0,nagato_defense_bonus-attack_damage)
            print(f"娜比娅对长门造成了 {effective_damage} 点伤害！")
        if action == 'defend':
            print("娜比娅选择了防御！")
            nabiya_defense_bonus = calculate_defense_value(NABIYA_DEFEND_DICE)
            print(f"娜比娅的防御值提升了 {nabiya_defense_bonus} 点！")
        if nagato_hp <= 0:
            print("\n长门被击败了！娜比娅获胜！")
            break
